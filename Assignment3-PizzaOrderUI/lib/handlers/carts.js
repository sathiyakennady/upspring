...
handlers.createCart = function(userId, callback) {
    ...
    if (userId) {
        ...
        //add cart to disc
        _data.create('carts', userId, function(cartData, err ) {
            if (!err) {
            callback(false);
        } else {
...
handlers.put = function(data, callback)  {
    ...
    if (cartId && mealId && token) {
        ...
        verifyToken(cartId, token, function(tokenNotValid ){
            if (!tokenNotValid) {
            //lookup user
            _data.read('carts', cartId, function(err, cartData) {
                ...
                if (!_meal && !remove) {
                    ...
                    _data.read('meals', mealId, function(err, mealData)  {
                        if (!err && mealData) {
                        ...
                        _data.update('carts', cartId, function(cartData, err)  {
                            if (!err) {
...
handlers.updateCartItems =function (cartData, mealId, reduce, remove) {
    var _meal = typeof (cartData.items.find(item => item.mealId == mealId)) !== 'undefined' ? cartData.items.find(item => item.mealId == mealId) : false;
